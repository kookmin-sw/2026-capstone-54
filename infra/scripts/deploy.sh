#!/usr/bin/env bash
#
# k3s production 배포 / 롤백 스크립트 (backend)
#
# 사용법:
...
run_migrate() {
  local image_tag="$1"
  local job_yml="${INFRA_DIR}/backend/jobs/migrate-job.yml"
...
run_collectstatic() {
  local image_tag="$1"
  local job_yml="${INFRA_DIR}/backend/jobs/collectstatic-job.yml"
...
if [[ "$TARGET" == "all" ]]; then
  echo "▶ 공통 매니페스트 적용 중..."
  kubectl apply -f "${INFRA_DIR}/common/"

  echo "▶ backend 매니페스트 적용 중..."
  # jobs/ 는 별도 처리하므로 --recursive=false
  kubectl apply -f "${INFRA_DIR}/backend/" --recursive=false
