...
class VideoAnalysisAggregator:
    ...
    def _generate_comment(
        ...
        )ral + avg_positive) * 100
io) * 100

        parts = []
        parts.append(f"시선이 정면을 유지한 비율이 {gaze_pct:.0f}%입니다.")
        parts.append(
            f"표정은 대체로 중립적이며 긍정 표현 비율은 {face_pct:.0f}%입니다."
        )
    ...
    def _generate_comment(
        ...
        """rule-based 영상 분석 코멘트 생성 (기획 REQ-2-7 템플릿)."""
        gaze_pct = (1 - gaze_ratio) * 100
        face_pct = (neutral_ratio + positive_ratviewRecordingTable.duration_ms,
                )
                .filter(InterviewRecordingTable.interview_session_id == session_id)
                .all()
            )
    ...
    @staticmethod
    def _load_gaze_data(session_id: str) -> dict[int, dict]:
        """InterviewTurn에서 gaze_away_count와 duration 정보를 로드한다."""
        with get_session() as db:
            recordings = (
                db.query(
                    InterviewRecordingTable.interview_turn_id,
    ...
    def _load_recordings(session_id: str) -> list:
        with get_session() as db:
            return (
                db.query(InterviewRecordingTable)
                .filter(InterviewRecordingTable.interview_session_id ==         video_score = round(gaze_score * 0.6 + face_score * 0.4)
        video_score = max(0, min(100, video_score))

        # 코멘트 생성
        comment = self._generate_