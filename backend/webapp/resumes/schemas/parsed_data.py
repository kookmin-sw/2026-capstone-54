"""이력서 분석 결과(parsed_data) 의 정규화된 스키마.

analysis-resume worker 의 LLM 구조화 출력 계약이자, backend 에서 apply_analysis_result
태스크가 입력을 검증할 때 사용하는 Pydantic 스키마.

SYNC: /Users/koa/005-capstone/analysis-resume/app/schemas/parsed_data.py 와 1:1 동기화한다.
변경 시 양쪽을 함께 수정한다.
"""
...
class ParsedResumeData(BaseModel):
  ...
  def from_raw(cls, data: dict[str, Any] | None) -> "ParsedResumeData":
    """분석 서비스에서 받은 원시 dict 를 정규화 스키마로 변환한다."""
    if not data:
      return cls()
    try:
      return cls.model_validate(data)
    except Exception:
      # 구 포맷 호환: skills 가 flat list 였던 경우
      safe = dict(data)
      if isinstance(safe.get("skills"), list):
        safe["skills"] = {"technical": safe["skills"], "soft": [], "tools": [], "languages": []}
      # career_level 등 제거된 필드가 원시 dict 에 남아 있어도 Pydantic 이 무시하도록 pop
