...
class CreateInterviewSessionMultipleActiveSessionsTests(TestCase):
  """create_interview_session 다중 active 세션 허용 검증."""
  ...
  def test_creates_session_when_in_progress_session_exists(self):
    """IN_PROGRESS 상태인 세션이 존재해도 새 세션을 정상적으로 생성한다."""
  ...
  def test_creates_session_when_paused_session_exists(self):
    """PAUSED 상태인 세션이 존재해도 새 세션을 정상적으로 생성한다."""
  ...
  def test_creates_multiple_in_progress_sessions_for_same_user(self):
    """동일 사용자에 대해 다중 IN_PROGRESS 세션을 동시에 생성할 수 있다."""
    first = create_interview_session(
      user=self.user,
      resume=self.resume,
      user_job_description=self.user_job_description,
      interview_session_type=InterviewSessionType.FOLLOWUP,
    ...
    second = create_interview_session(
      user=self.user,
      resume=self.resume,
      user_job_description=self.user_job_description,
      interview_session_type=InterviewSessionType.FOLLOWUP,
